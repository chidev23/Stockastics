  ...
  return (
    <SafeAreaView className="flex-1 bg-white">
      <View className="flex-1 justify-center px-6">
        ...
        {!!message && <Text className="mt-3 text-sm text-stockastics-green">{message}</Text>}
        <Pressable onPress={handleReset} disabled={busy} className="mt-5 items-center rounded-2xl bg-stockastics-blue py-4">
          {busy ? <ActivityIndicator color="#FFFFFF" /> : <Text className="font-bold text-white">Send reset email</Text>}
        </Pressable>
        <Link href="/auth/login" className="mt-5 text-center font-semibold text-stockastics-blue">Back to sign in</Link>
      </View>
    </SafeAreaView>
  );
}
