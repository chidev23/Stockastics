...
export default function LoginScreen() {
  const handleLogin = async () => { await setTestSession(); router.replace('/(main)/'); };
  const handleGoogleLogin = async () => { await setTestSession(); router.replace('/(main)/'); };
          ...
          <View className="flex-1 justify-center">
            <View className="mx-auto w-full max-w-[520px]">
              <View className="items-center">
                <View className="h-20 w-20 overflow-hidden rounded-2xl bg-black"><Image source={require('../../assets/icon.jpg')} className="h-full w-full" resizeMode="contain" accessibilityLabel="STOCKASTICS logo" /></View>
              ...
              <Pressable onPress={handleGoogleLogin} className="mt-8 h-14 flex-row items-center justify-center rounded-xl border border-slate-200 bg-white active:opacity-70"><GoogleLogo /><Text className="ml-3 text-base font-semibold text-slate-800">Log in with Google</Text></Pressable>
              ...
              <View className="gap-4"><TextInput placeholder="Email address" placeholderTextColor="#A1A1AA" autoCapitalize="none" autoComplete="email" keyboardType="email-address" className="h-14 rounded-xl border border-slate-200 bg-white px-4 text-base text-slate-900" /><TextInput placeholder="Password" placeholderTextColor="#A1A1AA" secureTextEntry autoComplete="password" className="h-14 rounded-xl border border-slate-200 bg-white px-4 text-base text-slate-900" /></View>
