...
const suggestions = ['What is the signal for TSLA?','Compare NVDA and AMD signals','What is the sentiment for Nigeria?','What events are happening today?','What stocks are trending right now?'];
...
export default function SticksAI() {
  ...
  const send = () => {
    if (!q.trim()) return;
    setMsgs((m) => [...m, { r: 'You', t: q.trim() }, { r: 'Sticks AI', t: 'I can help explain STOCKASTICS signals, market sentiment, news, events and portfolio information using available market data and proprietary analysis. Not financial advice. Do your own research.' }]);
    setQ('');
  };
    ...
    <SafeAreaView className="flex-1 bg-slate-50" edges={['top', 'bottom']}>
      <KeyboardAvoidingView className="flex-1" behavior={Platform.OS === 'ios' ? 'padding' : undefined} keyboardVerticalOffset={0}>
        ...
        <ScrollView className="flex-1" keyboardShouldPersistTaps="handled" keyboardDismissMode={Platform.OS === 'ios' ? 'interactive' : 'on-drag'} contentContainerClassName="px-5 pb-6 pt-5">
          ...
          {suggestions.map((x) => <Pressable key={x} onPress={() => setQ(x)} className="mb-2 rounded-xl border border-slate-200 bg-white px-4 py-3"><Text className="text-sm text-slate-700">{x}</Text></Pressable>)}
          {msgs.map((m, i) => <View key={i} className={`mt-3 rounded-2xl p-4 ${m.r === 'You' ? 'ml-8 bg-slate-200' : 'mr-4 border border-slate-200 bg-white'}`}><Text className="text-xs font-bold text-slate-500">{m.r}</Text><Text className="mt-2 text-sm leading-6 text-slate-800">{m.t}</Text></View>)}
        </ScrollView>
        <View className="border-t border-slate-200 bg-white px-5 pt-3" style={{ paddingBottom: bottomSafePadding }}><View className="flex-row rounded-2xl bg-slate-100 px-4 py-2"><TextInput value={q} onChangeText={setQ} onSubmitEditing={send} returnKeyType="send" placeholder="Ask Sticks AI..." placeholderTextColor="#94A3B8" className="min-h-10 flex-1 text-base text-slate-900"/><Pressable onPress={send} className="ml-2 h-10 w-10 items-center justify-center rounded-xl bg-blue-700"><Ionicons name="arrow-up" size={19} color="white" /></Pressable></View></View>
