...
export default function SticksAI() {
  ...
  const send = () => {
    ...
    setMsgs((m) => [
      ...m,
      { r: 'You', t: q.trim() },
      { r: 'Sticks AI', t: 'I can help explain STOCKASTICS signals, market sentiment, news, events and portfolio information using available market data and proprietary analysis. Not financial advice. Do your own research.' },
    ]);
    setQ('');
  };
      ...
      <KeyboardAvoidingView className="flex-1" behavior={Platform.OS === 'ios' ? 'padding' : 'height'} keyboardVerticalOffset={0}>
        ...
        <View className="border-t border-slate-200 bg-white px-5 pt-3" style={{ paddingBottom: bottomSafePadding }}>
          <View className="flex-row rounded-2xl bg-slate-100 px-4 py-2">
            <TextInput value={q} onChangeText={setQ} onSubmitEditing={send} returnKeyType="send" placeholder="Ask Sticks AI..." placeholderTextColor="#94A3B8" className="min-h-10 flex-1 text-base text-slate-900" />
            <Pressable onPress={send} className="ml-2 h-10 w-10 items-center justify-center rounded-xl bg-blue-700"><Ionicons name="arrow-up" size={19} color="white" /></Pressable>
          </View>
        </View>
