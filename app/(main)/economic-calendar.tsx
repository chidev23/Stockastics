...
export default function EconomicCalendar(){
  ...
  return <SafeAreaView className="flex-1 bg-slate-50" edges={['top']}><View className="flex-row items-center justify-between border-b border-slate-100 bg-white px-5 py-4"><View className="flex-row items-center"><MoreBackButton/><View className="ml-3"><Text style={label?{color:accent}:undefined} className="text-xl font-extrabold text-slate-900">{label?`${label} Economic Calendar`:'Economic Calendar'}</Text><Text className="mt-0.5 text-xs text-slate-500">{label?`Events relevant to the ${label} stock universe`:'Market-moving economic events'}</Text></View></View><Pressable><Ionicons name="options-outline" size={21} color="#2563EB"/></Pressable></View><ScrollView contentContainerClassName="px-5 pb-8 pt-5"><View className="mb-5 flex-row items-center justify-between"><Pressable className="rounded-xl bg-white px-4 py-3"><Text className="font-bold text-slate-800">‹ Previous</Text></Pressable><Text className="font-extrabold text-slate-900">Today ▾</Text><Pressable className="rounded-xl bg-white px-4 py-3"><Text className="font-bold text-slate-800">Next ›</Text></Pressable></View>{label?<View style={{backgroundColor:`${accent}10`,borderColor:`${accent}30`}} className="mb-5 rounded-2xl border p-4"><Text style={{color:accent}} className="font-extrabold">{label} scope active</Text><Text className="mt-1 text-xs leading-5 text-slate-600">Production calendar results should be filtered to events relevant to {label}-classified stocks and companies. This mode never falls back to the General stock universe.</Text></View>:null}<Text className="mb-3 text-lg font-extrabold text-slate-900">Upcoming Events</Text>{events.map(e=><Pressable key={e[1]} onPress={()=>alert(`${e[1]}\n${e[2]}\nImpact: ${e[3]}`)} className="mb-3 rounded-2xl border border-slate-200 bg-white p-4"><View className="flex-row justify-between"><Text className="font-extrabold text-slate-900">{e[0]}</Text><Text className={`text-xs font-extrabold ${e[3]==='HIGH'?'text-red-600':e[3]==='MEDIUM'?'text-amber-600':'text-emerald-600'}`}>{e[3]}</Text></View><Text className="mt-2 font-bold text-slate-800">{e[1]}</Text><Text className="mt-1 text-xs text-slate-500">{e[2]}</Text></Pressable>)}</ScrollView></SafeAreaView>
}
