...
const categories = [
  ...
  { key: 'ipo', label: 'IPO', icon: 'rocket-outline' as const },
  { key: 'buyback', label: 'Buyback', icon: 'repeat-outline' as const },
  { key: 'sentiment', label: 'Sentiment', icon: 'people-outline' as const },
  { key: 'ex-dividend', label: 'Ex-Div', icon: 'cash-outline' as const },
  { key: 'income', label: 'Income', icon: 'wallet-outline' as const },
];
...
export default function ReligiousSignalsScreen() {
  ...
  const category = categories.find((item) => item.key === params.category)?.key ?? 'retail';
  const categoryLabel = categories.find((item) => item.key === category)?.label ?? 'Retail';

  return (
    <SafeAreaView className="flex-1 bg-slate-50" edges={['top']}>
      <View className="flex-row items-center px-5 pt-4">
        <Pressable onPress={() => router.replace({ pathname: '/(main)/signals', params: { religious } })} className="mr-3 h-10 w-10 items-center justify-center rounded-full bg-white"><Ionicons name="arrow-back" size={20} color="#0F172A" /></Pressable>
        <View className="flex-1"><Text style={{ color: accent }} className="text-xs font-extrabold uppercase tracking-widest">{modeLabel} Signals</Text><Text className="mt-1 text-2xl font-extrabold text-slate-900">{categoryLabel} Signals</Text></View>
      </View>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} className="mt-5 max-h-20" contentContainerClassName="gap-2 px-5">
        {categories.map((item) => {
          ...
          return <Pressable key={item.key} onPress={() => router.replace({ pathname: '/(main)/religious-signals', params: { religious, category: item.key } })} className={`min-w-[88px] items-center rounded-2xl px-3 py-3 ${selected ? 'bg-emerald-600' : 'border border-slate-200 bg-white'}`}><Ionicons name={item.icon} size={20} color={selected ? '#FFFFFF' : accent} /><Text className={`mt-1 text-xs font-bold ${selected ? 'text-white' : 'text-slate-700'}`}>{item.label}</Text></Pressable>;
        })}
      </ScrollView>

      <ScrollView contentContainerClassName="px-5 pb-8 pt-5">
        <View style={{ backgroundColor: accent }} className="mb-5 rounded-3xl p-5"><Text className="text-xs font-extrabold tracking-widest text-white/80">{modeLabel.toUpperCase()} STOCK SCREENING</Text><Text className="mt-2 text-2xl font-extrabold text-white">{categoryLabel} BUY Signals</Text><Text className="mt-2 text-sm leading-5 text-white/90">Only {modeLabel}-classified stocks belonging to the {categoryLabel} signal category should appear in this feed.</Text></View>
