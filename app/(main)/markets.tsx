...
export default function MarketsScreen() {
  ...
  const filtered = useMemo(() => stocks.filter(s => (country === 'All' || s.country === country) && `${s.ticker} ${s.company}`.toLowerCase().includes(query.trim().toLowerCase())), [country, query]);
  ...
  return <SafeAreaView className="flex-1 bg-slate-50" edges={['top']}>
    <View className="px-5 pt-4"><Text className="text-2xl font-extrabold text-slate-900">Markets</Text><Text className="mt-1 text-sm text-slate-500">Explore stocks and market data by country.</Text>
