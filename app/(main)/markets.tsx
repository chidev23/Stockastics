...
// Demo records intentionally remain unclassified. Production stock APIs must
// return religious='halal' or religious='haram' before a record can enter the
// corresponding religious market universe.
const stocks: Stock[] = [
...
export default function MarketsScreen() {
  ...
  const filtered = useMemo(
    () => scopedStocks.filter((s) =>
      (country === 'All' || s.country === country) &&
      `${s.ticker} ${s.company}`.toLowerCase().includes(query.trim().toLowerCase())
    ),
    [country, query, scopedStocks]
  );

  const open = (s: Stock) => router.push({ pathname: '/stock-market-view', params: { ...s, ...({ religious } as Record<string, unknown>) } } as never);
  ...
  return <SafeAreaView className="flex-1 bg-slate-50" edges={['top']}>
    <View className="px-5 pt-4">
      <Text style={label ? { color: accent } : undefined} className="text-2xl font-extrabold text-slate-900">{label ? `${label} Markets` : 'Markets'}</Text>
      <Text className="mt-1 text-sm text-slate-500">{label ? `This is an isolated ${label} stock universe. Only API records explicitly classified as ${label} can appear here.` : 'Explore stocks and market data by country.'}</Text>
