...
export default function MarketsScreen() {
  ...
  return <SafeAreaView className="flex-1 bg-slate-50" edges={['top']}>
    ...
    <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ height: 58, flexGrow: 0 }} contentContainerClassName="items-center gap-2 px-5 py-1">{countries.map(c => <Pressable key={c} onPress={() => setCountry(c)} style={{ minHeight: 44, justifyContent: 'center' }} className={`rounded-full px-4 ${country === c ? 'bg-blue-700' : 'border border-slate-200 bg-white'}`}><Text className={`text-xs font-bold ${country === c ? 'text-white' : 'text-slate-700'}`}>{c}</Text></Pressable>)}</ScrollView>
