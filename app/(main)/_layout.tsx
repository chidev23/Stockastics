...
function StockasticsTabBar({ state, descriptors, navigation }: any) {
  ...
  return <View style={{ width: '100%', alignSelf: 'stretch', backgroundColor: '#FFFFFF', borderTopWidth: 1, borderTopColor: '#DDE5E1', paddingBottom: bottomInset, shadowColor: '#000000', shadowOffset: { width: 0, height: -2 }, shadowOpacity: 0.08, shadowRadius: 6, elevation: 10 }}><View style={{ width: '100%', alignSelf: 'stretch', height: 82, flexDirection: 'row', alignItems: 'stretch' }}>{PRIMARY_TABS.map((name, index) => { const route = state.routes.find((item: any) => item.name === name); if (!route) return null; const focused = activeName === name; const baseColor = NAV_COLORS[name]; const color = focused ? baseColor : `${baseColor}B3`; const options = descriptors[route.key]?.options ?? {}; const label = options.tabBarLabel ?? options.title ?? LABELS[name]; return <Pressable key={route.key} accessibilityRole="tab" accessibilityState={{ selected: focused }} accessibilityLabel={String(label)} onPress={() => { const event = navigation.emit({ type: 'tabPress', target: route.key, canPreventDefault: true }); if (event.defaultPrevented) return; if (name === 'index' || name === 'more' || !religiousMode) navigation.navigate(route.name); else navigation.navigate(route.name, { religious: religiousMode }); }} style={({ pressed }) => ({ width: '20%', flexGrow: 0, flexShrink: 0, alignItems: 'center', justifyContent: 'flex-start', paddingTop: 7, paddingBottom: 5, paddingHorizontal: 1, opacity: pressed ? 0.65 : 1, borderLeftWidth: index === 0 ? 0 : 1, borderLeftColor: '#EEF2F0' })}><View style={{ width: 48, height: 42, alignItems: 'center', justifyContent: 'center', borderRadius: 13, backgroundColor: focused ? `${baseColor}18` : 'transparent' }}><Ionicons name={ICONS[name]} size={25} color={color} /></View><Text numberOfLines={1} allowFontScaling={false} style={{ marginTop: 4, fontSize: 12, lineHeight: 16, fontWeight: focused ? '800' : '600', color, textAlign: 'center' }}>{label}</Text></Pressable>; })}</View></View>;
}
