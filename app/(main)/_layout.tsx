...
function StockasticsTabBar({ state, descriptors, navigation }: any) {
  ...
  return <View style={{ width: '100%', alignSelf: 'stretch', backgroundColor: '#FFFFFF', borderTopWidth: 1, borderTopColor: '#DDE5E1', paddingBottom: bottomInset, shadowColor: '#000000', shadowOffset: { width: 0, height: -2 }, shadowOpacity: 0.08, shadowRadius: 6, elevation: 10 }}>
    <View style={{ width: '100%', alignSelf: 'stretch', height: 82, flexDirection: 'row', alignItems: 'stretch' }}>
      {PRIMARY_TABS.map((name, index) => {
        ...
        return <Pressable key={route.key} accessibilityRole="tab" accessibilityState={{ selected: focused }} accessibilityLabel={String(label)} onPress={() => {
          ...
          if (name === 'index' || name === 'more' || !religiousMode) navigation.navigate(route.name);
          else navigation.navigate(route.name, { religious: religiousMode });
        }} style={({ pressed }) => ({ width: '20%', flexGrow: 0, flexShrink: 0, alignItems: 'center', justifyContent: 'flex-start', paddingTop: 7, paddingBottom: 5, paddingHorizontal: 1, opacity: pressed ? 0.65 : 1, borderLeftWidth: index === 0 ? 0 : 1, borderLeftColor: '#EEF2F0' })}>
