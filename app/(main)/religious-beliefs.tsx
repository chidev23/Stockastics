...
export default function ReligiousBeliefsScreen(){
  return <SafeAreaView className="flex-1 bg-slate-50" edges={['top']}>
    <View className="flex-row items-center border-b border-slate-100 bg-white px-5 py-4">
      <Pressable accessibilityRole="button" accessibilityLabel="Back to Home" onPress={()=>router.replace('/(main)/index')} className="h-10 w-10 items-center justify-center rounded-full bg-slate-50" hitSlop={8}><Ionicons name="arrow-back" size={22} color="#0F172A"/></Pressable>
